...
import 'package:musiz_player/api/functions.dart';
import 'package:musiz_player/api/nav.dart';
import 'package:musiz_player/utils/downlod.dart';
import 'package:hive_flutter/hive_flutter.dart';
import 'package:http/http.dart';
import 'package:logging/logging.dart';
import 'package:musiz_player/utils/pprint.dart';
...
class YtMusicService {
  ...
  initLanguage() async {
    context!['context']['client']['hl'] = Hive.box(
      'settings',
    ).get('language', defaultValue: 'en');
    context!['context']['client']['gl'] = Hive.box(
      'settings',
    ).get('locationCode', defaultValue: 'IN');
  }
  ...
  Future<Map<String, dynamic>> getArtistDetails(String id) async {
    ...
    try {
      ...
      for (final item in finalResults) {
        ...
        final String image =
            nav(item, [mRLIR, ...thumbnails, 0, 'url']).toString();
        final String title =
            nav(item, [
              mRLIR,
              'flexColumns',
