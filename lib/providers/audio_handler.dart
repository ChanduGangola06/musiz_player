...
import 'package:musiz_player/utils/history.dart';
...
class MyAudioHandler extends BaseAudioHandler with QueueHandler, SeekHandler {
  ...
  _loadEquilizer() async {
    await _loudnessEnhancer.setEnabled(
      Hive.box('settings').get('loudnessEnabled', defaultValue: false) as bool,
    );
    await _loudnessEnhancer.setTargetGain(
      Hive.box('settings').get('loudness', defaultValue: 0.0),
    );

    await _equalizer.setEnabled(
      Hive.box('settings').get('equalizerEnabled', defaultValue: false) as bool,
    );
    _equalizer.parameters.then((value) async {
      _equalizerParams ??= value;

      final List<AndroidEqualizerBand> bands = _equalizerParams!.bands;
      await Future.forEach(bands, (e) {
        final gain =
            Hive.box(
                  'settings',
                ).get('equalizerBand${e.index}', defaultValue: 0.0)
                as double;
  ...
  Future<UriAudioSource> _createAudioSource(MediaItem mediaItem) async {
    String? u = await GetIt.I<PlaybackCache>().getFile(
