...
import 'package:musiz_player/api/extensions.dart';
import 'package:musiz_player/models/setting_item.dart';
import 'package:musiz_player/providers/media_manager.dart';
import 'package:musiz_player/providers/theme_manager.dart';
import 'package:musiz_player/screens/settings/equalizer_screen.dart';
import 'package:musiz_player/ui/colors.dart';
import 'package:musiz_player/ui/text_styles.dart';
import 'package:musiz_player/utils/format_duration.dart';
import 'package:musiz_player/utils/playback_cache.dart';
import 'package:musiz_player/utils/snackbar.dart';
    ...
    trailing: (context) {
      ...
      return mediaManager.timerDuration != null
          ? Wrap(
            ...
            children: [
              Text(
                formatDuration(mediaManager.timerDuration!),
    ...
    trailing: (context) {
      return ValueListenableBuilder(
        ...
        builder: (context, value, child) {
          return Text(
            value.get('locationName', defaultValue: 'Palistine'),
...
List<SettingItem> appAppearenceSettingDataList(BuildContext context) => [
  SettingItem(
    ...
    trailing: (context) {
      return Text(
        context.watch<ThemeManager>().language['name'],
        style: smallTextStyle(context),
      );
    },
    onTap: (context) {
  ...
  SettingItem(
    ...
    trailing:
        (context) => CupertinoSwitch(
          ...
          onChanged:
              (value) => context.read<ThemeManager>().toggleTextDirection(),
        ),
  ),
  SettingItem(
  ...
  SettingItem(
    ...
    trailing:
        (context) => CupertinoSwitch(
          ...
          onChanged:
              (value) => context.read<ThemeManager>().toggleMaterialTheme(),
        ),
  ),
];
                        ...
                        ...langs.map((lang) {
                          return ListTile(
                            ...
                            trailing: Checkbox(
                              ...
                              onChanged: (val) {
                                if (val != null) {
                                  context.read<ThemeManager>().toggleLanguage(
                                    lang,
                                    val,
                                  );
                                }
  ...
  SettingItem(
    ...
    trailing: (context) {
      ...
      return StreamBuilder(
        stream: event,
        builder:
            (context, snapshot) => DropdownButton2<int>(
              style: textStyle(context, bold: false).copyWith(fontSize: 16),
              underline: const SizedBox(),
              value:
                  snapshot.data?.value ??
  ...
  SettingItem(
    ...
    trailing: (context) {
      ...
      return StreamBuilder(
        stream: event,
        builder:
            (context, snapshot) => DropdownButton2<String>(
              style: textStyle(context, bold: false).copyWith(fontSize: 16),
              underline: const SizedBox(),
              value:
                  snapshot.data?.value ??
