...
download(MediaItem song) async {
  ...
  String path =
      Platform.isAndroid
          ...
          : (await getDownloadsDirectory())!.path.replaceAll(
            'Downloads',
            'Music',
          );
  ...
  if (song.extras!['provider'] == 'youtube') {
    ...
    int downloadQuality = Hive.box(
      'settings',
    ).get('downloadQuality', defaultValue: 160);
    String url = song.extras!['url'].toString().replaceAll(
      RegExp('_92|_160|_320'),
      '_$downloadQuality',
    );
...
Future<MediaItem> processSong(Map song) async {
  ...
  if (downloaded != null &&
    ...
    if (song['provider'] != 'youtube') {
      int streamingQuality = Hive.box(
        'settings',
      ).get('streamingQuality', defaultValue: 160);
      song['url'] = song['url'].toString().replaceAll(
        '_96',
        '_$streamingQuality',
      );
    }
