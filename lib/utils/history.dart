...
Future<void> addSongHistory(Map song) async {
  bool isPlaybackCache = Hive.box(
    'settings',
  ).get('playbackCache', defaultValue: false);

  Map? isDownloaded = Hive.box('downloads').get(song['id']);
  if (isPlaybackCache && isDownloaded == null) {
    bool isLocal = await GetIt.I<PlaybackCache>().existedInLocal(
      url: song['url'],
    );
    if (!isLocal) {
  ...
  bool isEnabled = Hive.box(
    'settings',
  ).get('playbackHistory', defaultValue: true);
  if (!isEnabled) return;
